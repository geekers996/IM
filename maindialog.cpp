...
void MainDialog::paintEvent(QPaintEvent *event)
{
    ...
    painter.drawPixmap(rect(), QPixmap(":/new/prefix1/images/bgpng2.png"), QRect());
...
void MainDialog::UpdateClientProgressFunc(qint64 numBytes) // 文件传输及显示进度条更新
{
    ...
    {
        m_LocalFiles->close(); // 没有数据发送则直接关闭文件
    }
...
void MainDialog::on_pushButton_SendMsg_clicked()
{
    ...
    if(strMsg.isEmpty())
    {
        QMessageBox::critical(this, "错误", "发送到服务器的数据信息不能为空", QMessageBox::Yes);
